...
Embed video files in your presentations with `Podium.add_movie/4`. The video
appears as a clickable player that viewers can play during a slideshow.

```elixir
video_data = File.read!("quarterly_recap.mp4")

{prs, slide} = Podium.add_movie(prs, slide, video_data,
...
`Podium.add_movie/4` takes the presentation, slide, video binary, and options.
...
slide = Podium.set_placeholder(slide, :title, "Product Demo")
...
Podium.save(prs, "product_demo.pptx")
```
